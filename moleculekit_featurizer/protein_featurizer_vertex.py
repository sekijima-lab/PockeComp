...
def default_featurizer(pdb_path, mol_path):
    try:
        prot = Molecule(pdb_path)
        
        #mol_path = glob.glob(pdb_path.replace("_clean.pdb","_lig_*.pdb"))
        ...
        logger.debug(f"{pdb_path}:pocket_resid_chain_pairs={pocket_resid_chain_pairs}")
        ...
        logger.debug(f"{pdb_path}:check before saving {set([ (receptor_residue_number[i].item(),prot.chain[i]) for i in range(len(receptor_pocket_flag)) if receptor_pocket_flag[i] == 1])}")
...
def extract_number(filename):
    pattern = r'[A-Za-z0-9]{4}_lig_(\d+)\.pdb'
...
def featurizer_wrapper(args):
    try:
        pdb_path, mol_path = args
        os.makedirs("/".join(pdb_path.split("/")[:-1]).replace("/Vertex/","/processed/Vertex/"),exist_ok=True)
        logger.debug("convert pdb file : "+pdb_path+" "+mol_path)
        number = extract_number(os.path.basename(mol_path))
        coords,pocket_flag,residue_number,channels,ligand_coords,flag,chains = default_featurizer(pdb_path,mol_path)
        if dist==8:
            save_path = pdb_path.replace("/Vertex/","/processed/Vertex/").replace("_clean.pdb",f"_site_{number}.pickle")
        else:
            save_path = pdb_path.replace("/Vertex/","/processed/Vertex/").replace("_clean.pdb",f"_site_{number}_{dist}.pickle")
        if int(sys.argv[3]) != 2:
        ...
        if os.path.isfile(save_path):
            os.remove(save_path)
        ligand_save_path = mol_path.replace("/Vertex/","/processed/Vertex/").replace(".pdb",".pickle")
        if coords is None:
            return False
        torch.save({
            "coords":coords,
        ...
        logger.debug(f"{traceback.format_exc()}, cannot save protein and ligand info {pdb_path, mol_file}")
        return False

if __name__ == "__main__":
    utc_time = datetime.now()
    execution_time_str = (utc_time).strftime('%Y%m%d_%H%M')
    log_folder = f"./logs/vertex{execution_time_str}_{sys.argv[3]}"
    ...
    Vertex_PATH = sys.argv[1] #path to /TOUGH-M1/
    if len(sys.argv) > 2:
        dist = int(sys.argv[2])
    os.makedirs(Vertex_PATH.replace("/Vertex/","/processed/Vertex/"), exist_ok=True)
    pdb_files = search_pdb_files(Vertex_PATH)
    new_pdb_files = []
    for pdb_file in pdb_files:
        for mol_file in glob.glob(pdb_file.replace("_clean.pdb","_lig_*.pdb")):
            new_pdb_files.append((pdb_file,mol_file))
    ...
    logger.debug(f"Process files : {SUCCESS_COUNT}/{len(pdb_files)} {time()-start}s")
