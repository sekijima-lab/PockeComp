...
def search_pdb_files(directory):
    ...
    # 指定されたディレクトリ内のファイルを走査します
    for folder in os.listdir(directory):
        #logger.debug(folder)
        matching_files.append(os.path.join(directory,folder,folder.split("/")[-1]+".pdb"))
...
def default_featurizer(pdb_path):
    try:
        ...
        mol = Molecule(pdb_path.replace(".pdb","00.pdb"))
...
def featurizer_wrapper(pdb_path):
    os.makedirs("/".join(pdb_path.split("/")[:-1]).replace("/TOUGH-M1/","/processed/TOUGH-M1/"),exist_ok=True)
    ...
    if dist==8:
        save_path = pdb_path.replace("/TOUGH-M1/","/processed/TOUGH-M1/").replace(".pdb",f".pickle")
    else:
        save_path = pdb_path.replace("/TOUGH-M1/","/processed/TOUGH-M1/").replace(".pdb",f"_{dist}.pickle")
    if int(sys.argv[3]) != 2:
    ...
    if os.path.isfile(save_path):
        os.remove(save_path)
    ligand_save_path = pdb_path.replace("/TOUGH-M1/","/processed/TOUGH-M1/").replace(".pdb","_ligand.pickle")
    if coords is None:
        return False
    torch.save({
        "coords":coords,
...
if __name__ == "__main__":
    ...
    log_folder = f"./logs/toughm1{execution_time_str}_{sys.argv[3]}"
    ...
    TOUGH_M1_PATH = sys.argv[1] #path to /TOUGH-M1/
    if len(sys.argv) > 2:
        dist = int(sys.argv[2])
    os.makedirs(TOUGH_M1_PATH.replace("/TOUGH-M1/","/processed/TOUGH-M1/"),exist_ok=True)
    pdb_files = search_pdb_files(os.path.join(TOUGH_M1_PATH,"TOUGH-M1_dataset/"))
